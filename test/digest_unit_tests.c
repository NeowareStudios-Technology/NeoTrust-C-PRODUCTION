...
//-if header matches expected content, test passes
int CreateBaseSigFile_test()
{
    FILE *sigFilePointer;
    char dirName[] = TEST_DIR_NAME;
    char expectedSigContents[] = "";
    long sigFileLength;
    char *actualSigContents;
    char *metaInfDirPath = malloc(256);
    char *sigFilePath = malloc(256);

    strcpy(metaInfDirPath, dirName);
    strcat(metaInfDirPath, "/META-INF");
    mkdir(metaInfDirPath, 0700);

    strcpy(sigFilePath, dirName);
    strcat(sigFilePath, "/META-INF/tempSig");
    
    //create base signature file
    sigFilePointer = CreateBaseSigFile(metaInfDirPath);

     //read contents of base signature file into string
    if (!sigFilePointer)
        printf("(SaveFileNameAndDigestToSig) tesfiles/signature file coud not be opened to read");
    sigFileLength = getFileLength(sigFilePointer);
    actualSigContents = (char *)malloc((sigFileLength+1)*sizeof(char)); // Enough memory for file + \0
    fread(actualSigContents, sigFileLength, 1, sigFilePointer); // Read in the entire file
    actualSigContents[sigFileLength] = '\0';
    fclose(sigFilePointer); // Close the file

    remove(sigFilePath);
    rmdir(metaInfDirPath);

    //compare contents of created base signature file and expected contents, if they don't match the test fails
    if (strcmp(expectedSigContents, actualSigContents)!= 0)
    {
        printf("3) CreateBaseSigFile_test FAILED\n");
        free(metaInfDirPath);
        free(sigFilePath);
        free(actualSigContents);
        return 1;
    }
    ...
    printf("3) CreateBaseSigFile_test passed\n");
...
int CreateDigestsAndMetaInfEntries_test()
{
    ...
    CreateDigestsAndMetaInfEntries(dirName, manifestFilePointer, sigFilePointer); 
    
    rewind(manifestFilePointer);
    rewind(sigFilePointer);
    ...
    sigFilePointer = fopen(sigFilePath, "r");
    if (!sigFilePointer)
        printf("(CreateDigestsAndMetaInfEntries) testdir/tempSig file coud not be opened to read");
    sigFileLength = getFileLength(sigFilePointer);
    actualSigContents = (char *)malloc((sigFileLength+1)*sizeof(char)); // Enough memory for file + \0
    fread(actualSigContents, sigFileLength, 1, sigFilePointer); // Read in the entire file
    actualSigContents[sigFileLength] = '\0';
    ...
    if (strcmp(expectedManifestContents, actualManifestContents) != 0 && strcmp(expectedSigContents, actualSigContents) != 0)
    {
...
int GenerateFullManifestDigestAndSaveInSigFile_test()
{
    ...
    strcpy(sigFilePath, dirName);
    strcat(sigFilePath, "/META-INF/neotrust.sf");

    manifestFilePointer = CreateBaseManifestFile(metaInfDirPath, manifestFileName, pubKeyPlaceholder);
    sigFilePointer = CreateBaseSigFile(metaInfDirPath);
    CreateDigestsAndMetaInfEntries(dirName, manifestFilePointer, sigFilePointer); 

    finalSigFilePointer = GenerateFullManifestDigestAndSaveInSigFile(metaInfDirPath, sigFileName, manifestFilePointer, sigFilePointer);
    ...
    finalSigFileLength = getFileLength(finalSigFilePointer);
    actualFinalSigContents = (char *)malloc((finalSigFileLength+1)*sizeof(char)); // Enough memory for file + \0
    fread(actualFinalSigContents, finalSigFileLength, 1, finalSigFilePointer); // Read in the entire file
    actualFinalSigContents[finalSigFileLength] = '\0';
    fclose(sigFilePointer); // Close the file

    //uncomment to print results for this test
    //printf("\n\nEXPECTED\n%s\n\n", expectedFinalSigContents);
    //printf("\n\nACTUAL\n%s\n\n", actualFinalSigContents);

    remove(manifestFilePath);
    remove(sigFilePath);
    rmdir(metaInfDirPath);

    if (strcmp(expectedFinalSigContents, actualFinalSigContents) != 0)
    {
        printf("5) GenerateFullManifestDigestAndSaveInSigFile_test FAILED\n");
        free(metaInfDirPath);
        free(manifestFilePath);
        free(sigFilePath);
        free(actualFinalSigContents);
        return 1;
    }
...
int main()
{
    ...
    testStatuses[2] = CreateBaseSigFile_test();
